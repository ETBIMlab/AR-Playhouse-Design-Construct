...
public class ShiftScale : MonoBehaviour
{
    ...
    private Vector scaleChange, positionChange; 
    // Use this for initialization
    void Awake()
    {
        //create cube at origin
        cube = GameObject.CreatePrimitive(PrimitiveType.Cube);
        cube.transform.position = new Vector(0, 0, 0);

        //create plane and adjust down by 0.5
        GameObject plane = GameObject.CreatePrimitive(PrimitiveType.Plane);
        plane.transform.position = new Vector(0, -0.5f, 0);

        scaleChange = new Vector(-0.01f, -0.01f, -0.01f);
        positionChange = new Vector(0.0f, -0.005f, 0.0f);
    }
    void Update()
    {
